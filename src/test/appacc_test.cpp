...
bool CheckAppAcct(int64_t opValue[]) {
    ...
    contractAcct.bcoinBalance = 100 * COIN;  //这里将TX中100 COIN先充值到合约账户中，扮演系统操作账户角色
...
BOOST_AUTO_TEST_CASE(key_test1)
 {
    ...
    BOOST_CHECK(AccCount.GetbcoinBalance() == 0);            //因为操作符全是加冻结的钱所以自由金额必须是0
    ...
    {           //合并第一个
        ...
        BOOST_CHECK(AccCount.GetbcoinBalance() == OpArry[0].GetUint64Value());;                         //合并后自由金额必须没有问题
    }
    ...
    {                       //减去全部
        ...
        BOOST_CHECK(AccCount.Operate(OpArry2));                             //执行所有的操作符合
        BOOST_CHECK(AccCount.GetbcoinBalance() == 0);;                           //钱必须可以核对
    }

    {
        ...
        BOOST_CHECK(AccCount.Operate(OpArry2));                             //执行所有的操作符合
        BOOST_CHECK(AccCount.GetbcoinBalance() == OpArry[0].GetUint64Value());                //加上后 就回来了
    }

    AccCount.AutoMergeFreezeToFree(maxtimeout);                     //全部合并
    printf("%lu, %lu\n", AccCount.GetbcoinBalance(), allmony);
    BOOST_CHECK(AccCount.GetbcoinBalance() == allmony);                //余额平账
