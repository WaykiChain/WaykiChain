...
public:
	...
	bool submitaccountregistertx(const std::string &addr,const int height)
	{
		//CommanRpc
		char caddr[64] = { 0 };
		strncpy(caddr, addr.c_str(), sizeof(caddr)-1);
		...
		const char *argv[] = { "rpctest", "submitaccountregistertx", caddr, fee, height};
		int argc = sizeof(argv)/sizeof(char*);
...
//		strncpy(ccontract, contract.c_str(), sizeof(contract)-1);
	...
	bool DeployContractTx(const std::string &addr, const std::string &script, const int height)
	{
		...
		const char *argv[] = { "rpctest", "submitcontractdeploytx", caddr, csript, fee, height};
		int argc = sizeof(argv)/sizeof(char*);
...
BOOST_FIXTURE_TEST_CASE(block_normaltx_and_regaccounttx,CMinerTest)
{
	...
	{
		nCurHeight = height;
		BOOST_REQUIRE(submitaccountregistertx(destaddr, height));
		{
