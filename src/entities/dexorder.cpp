...
string CDEXOrderDetail::ToString() const {
    return strprintf(
            ...
            " asset_amount=%lu, price=%lu, fee_ratio=%f%%, tx_cord=%s, user_regid=%s, total_deal_coin_amount=%llu,"
            " total_deal_asset_amount=%llu",
            GetOrderGenTypeName(generate_type), GetOrderSideName(order_side), GetOrderTypeName(order_type),
            coin_symbol, asset_symbol, coin_amount, asset_amount, price, GetFeeRatioF(), tx_cord.ToString(),
...
void CDEXOrderDetail::ToJson(json_spirit::Object &obj) const {
    ...
    obj.push_back(Pair("fee_ratio",                  GetFeeRatioF()));
...
double CDEXOrderDetail::GetFeeRatioF() const {
    return fee_ratio / (double) PRICE_BOOST;
}

///////////////////////////////////////////////////////////////////////////////
// class CDEXSysOrder

shared_ptr<CDEXOrderDetail> CDEXSysOrder::CreateBuyLimitOrder(const CTxCord &txCord,
                                                              const TokenSymbol &coinTypeIn,
...
shared_ptr<CDEXOrderDetail> CDEXSysOrder::CreateSellLimitOrder(const CTxCord& txCord,
                                                               const TokenSymbol& coinTypeIn,
                                                               const AssetSymbol& assetTypeIn,
                                                               uint64_t assetAmountIn,
                                                               uint64_t priceIn) {
