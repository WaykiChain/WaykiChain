...
static bool GetKeyId(const CAccountViewCache &view, vector<unsigned char> &ret, CKeyID &KeyId) {
    if (ret.size() == 6) {
        CRegID reg(ret);
...
static int ExSha256Func(lua_State *L) {
    ...
    string data(retdata.at(0).get()->begin(), retdata.at(0).get()->end());
    SHA256_CTX sha256;
    SHA256_Init(&sha256);
    SHA256_Update(&sha256, data.c_str(), data.size());
    SHA256_Final(hash, &sha256);

    vector<unsigned char> tep(hash, hash + SHA256_DIGEST_LENGTH);
    // LogPrint("vm", "data:%s, hash:%s\n", data.c_str(), HexStr(tep.begin(), tep.end()));

    return RetRstToLua(L, tep);
}

/**
 *unsigned short Des(void const* pdata, unsigned short len, void const* pkey,
 *unsigned short keylen, bool IsEn, void * const pOut,unsigned short poutlen)
 * 这个函数式从中间层传了三个个参数过来:
 * 1.第一个是要被加密数据或者解密数据
 * 2.第二格式加密或者解密的key值
 * 3.第三是标识符，是加密还是解密
...
/**
 *bool SignatureVerify(void const* data, unsigned short datalen, void const*
 key, unsigned short keylen, void const* phash, unsigned short hashlen)
 * 这个函数式从中间层传了三个个参数过来:
 * 1.第一个是签名的数据
...
static int ExVerifySignatureFunc(lua_State *L) {
    ...
    if (!GetDataTableVerifySignature(L, retdata) || retdata.size() != 3 || retdata.at(1).get()->size() != 33 || retdata.at(2).get()->size() != 32) {
        return RetFalse(string(__FUNCTION__) + "para  err !");
    }
...
/**
 *void LogPrint(const void *pdata, const unsigned short datalen,PRINT_FORMAT
 *flag ) 这个函数式从中间层传了两个个参数过来:
...
/**
 *unsigned short GetAccounts(const unsigned char *txhash, void* const paccount,
 *unsigned short maxlen) 这个函数式从中间层传了一个参数过来: 1.第一个是 hash
 */
...
/**
 *unsigned short GetAccountPublickey(const void* const accounid,void * const
 *pubkey,const unsigned short maxlength) 这个函数式从中间层传了一个参数过来:
 * 1.第一个是 账户id,六个字节
 */
static int ExGetAccountPublickeyFunc(lua_State *L) {
    vector<std::shared_ptr<vector<unsigned char>>> retdata;
    if (!GetArray(L, retdata) || retdata.size() != 1 || !(retdata.at(0).get()->size() == 6 || retdata.at(0).get()->size() == 34)) {
...
/**
 *bool QueryAccountBalance(const unsigned char* const account,Int64* const
 *pBalance) 这个函数式从中间层传了一个参数过来: 1.第一个是 账户id,六个字节
 */
...
/**
 *bool WriteDataDB(const void* const key,const unsigned char keylen,const void *
 *const value,const unsigned short valuelen,const unsigned long time)
 * 这个函数式从中间层传了三个个参数过来:
 * 1.第一个是 key值
 * 2.第二个是value值
 */
...
/**
 *unsigned short ReadDataValueDB(const void* const key,const unsigned char
 *keylen, void* const value,unsigned short const maxbuffer)
 * 这个函数式从中间层传了一个参数过来:
 * 1.第一个是 key值
 */
...
/**
 *bool ModifyDataDBVavle(const void* const key,const unsigned char keylen, const
 *void* const pvalue,const unsigned short valuelen) 中间层传了两个参数
 * 1.第一个是 key
 * 2.第二个是 value
 */
...
/**
 *bool GetContractData(const void* const scriptID,void* const pkey,short
 *len,void* const pvalve,short maxlen) 中间层传了两个个参数 1.脚本的id号
 * 2.数据库的key值
 */
