...
namespace eosio { namespace vm {
   ...
   struct interpret_visitor : base_visitor {
      ...
      [[gnu::always_inline]] inline void operator()(const br_table_t& op) {
         const auto& in = context.pop_operand();
         const auto& entry = op.table[std::min(in.to_ui32(), op.size)]; 
      ...
      [[gnu::always_inline]] inline void operator()(const call_indirect_t& op) {
         const auto& index = context.pop_operand();
         context.call(context.table_elem(index.to_ui32()));
      }
      ...
      [[gnu::always_inline]] inline void operator()(const i64_ne_t& op) {
         ...
         //const auto& rhs = context.pop_operand();
         auto&       lhs = context.peek_operand();
         //std::cout << "i64_ne_t rhs:" << rhs.to_ui64() << "lhs:" << lhs.to_ui64() << std::endl;
         lhs             = i32_const_t{ lhs.to_ui64() != rhs.to_ui64() };
      }
      [[gnu::always_inline]] inline void operator()(const i64_lt_s_t& op) {
         ...
         const auto& rhs = context.pop_operand();
         auto&       lhs = context.peek_operand();
         lhs             = i32_const_t{ lhs.to_i64() < rhs.to_i64() };
      }
      ...
      [[gnu::always_inline]] inline void operator()(const f32_eq_t& op) {
         ...
         if constexpr (use_softfloat)
            lhs = i32_const_t{ (uint32_t)_eosio_f32_eq(lhs.to_f32(), rhs.to_f32()) };
         else
            lhs = i32_const_t{ (uint32_t)(lhs.to_f32() == rhs.to_f32()) };
      }
      [[gnu::always_inline]] inline void operator()(const f32_ne_t& op) {
         ...
         auto&       lhs = context.peek_operand();
         if constexpr (use_softfloat)
            lhs = i32_const_t{ (uint32_t)_eosio_f32_ne(lhs.to_f32(), rhs.to_f32()) };
         else
            lhs = i32_const_t{ (uint32_t)(lhs.to_f32() != rhs.to_f32()) };
      }
      [[gnu::always_inline]] inline void operator()(const f32_lt_t& op) {
         ...
         auto&       lhs = context.peek_operand();
         if constexpr (use_softfloat)
            lhs = i32_const_t{ (uint32_t)_eosio_f32_lt(lhs.to_f32(), rhs.to_f32()) };
         else
            lhs = i32_const_t{ (uint32_t)(lhs.to_f32() < rhs.to_f32()) };
      }
      [[gnu::always_inline]] inline void operator()(const f32_gt_t& op) {
         ...
         auto&       lhs = context.peek_operand();
         if constexpr (use_softfloat)
            lhs = i32_const_t{ (uint32_t)_eosio_f32_gt(lhs.to_f32(), rhs.to_f32()) };
         else
            lhs = i32_const_t{ (uint32_t)(lhs.to_f32() > rhs.to_f32()) };
      }
      [[gnu::always_inline]] inline void operator()(const f32_le_t& op) {
         ...
         auto&       lhs = context.peek_operand();
         if constexpr (use_softfloat)
            lhs = i32_const_t{ (uint32_t)_eosio_f32_le(lhs.to_f32(), rhs.to_f32()) };
         else
            lhs = i32_const_t{ (uint32_t)(lhs.to_f32() <= rhs.to_f32()) };
      }
      [[gnu::always_inline]] inline void operator()(const f32_ge_t& op) {
         ...
         auto&       lhs = context.peek_operand();
         if constexpr (use_softfloat)
            lhs = i32_const_t{ (uint32_t)_eosio_f32_ge(lhs.to_f32(), rhs.to_f32()) };
         else
            lhs = i32_const_t{ (uint32_t)(lhs.to_f32() >= rhs.to_f32()) };
      }
      [[gnu::always_inline]] inline void operator()(const f64_eq_t& op) {
         ...
         auto&       lhs = context.peek_operand();
         if constexpr (use_softfloat)
            lhs = i32_const_t{ (uint32_t)_eosio_f64_eq(lhs.to_f64(), rhs.to_f64()) };
         else
            lhs = i32_const_t{ (uint32_t)(lhs.to_f64() == rhs.to_f64()) };
      }
      [[gnu::always_inline]] inline void operator()(const f64_ne_t& op) {
         ...
         auto&       lhs = context.peek_operand();
         if constexpr (use_softfloat)
            lhs = i32_const_t{ (uint32_t)_eosio_f64_ne(lhs.to_f64(), rhs.to_f64()) };
         else
            lhs = i32_const_t{ (uint32_t)(lhs.to_f64() != rhs.to_f64()) };
      }
      [[gnu::always_inline]] inline void operator()(const f64_lt_t& op) {
         ...
         auto&       lhs = context.peek_operand();
         if constexpr (use_softfloat)
            lhs = i32_const_t{ (uint32_t)_eosio_f64_lt(lhs.to_f64(), rhs.to_f64()) };
         else
            lhs = i32_const_t{ (uint32_t)(lhs.to_f64() < rhs.to_f64()) };
      }
      [[gnu::always_inline]] inline void operator()(const f64_gt_t& op) {
         ...
         auto&       lhs = context.peek_operand();
         if constexpr (use_softfloat)
            lhs = i32_const_t{ (uint32_t)_eosio_f64_gt(lhs.to_f64(), rhs.to_f64()) };
         else
            lhs = i32_const_t{ (uint32_t)(lhs.to_f64() > rhs.to_f64()) };
      }
      [[gnu::always_inline]] inline void operator()(const f64_le_t& op) {
         ...
         auto&       lhs = context.peek_operand();
         if constexpr (use_softfloat)
            lhs = i32_const_t{ (uint32_t)_eosio_f64_le(lhs.to_f64(), rhs.to_f64()) };
         else
            lhs = i32_const_t{ (uint32_t)(lhs.to_f64() <= rhs.to_f64()) };
      }
      [[gnu::always_inline]] inline void operator()(const f64_ge_t& op) {
         ...
         auto&       lhs = context.peek_operand();
         if constexpr (use_softfloat)
            lhs = i32_const_t{ (uint32_t)_eosio_f64_ge(lhs.to_f64(), rhs.to_f64()) };
         else
            lhs = i32_const_t{ (uint32_t)(lhs.to_f64() >= rhs.to_f64()) };
      }
      ...
      [[gnu::always_inline]] inline void operator()(const i32_div_s_t& op) {
         ...
         const auto& rhs = context.pop_operand();
         auto&       lhs = context.peek_operand().to_i32();
         EOS_WB_ASSERT(rhs.to_i32() != 0, wasm_interpreter_exception, "i32.div_s divide by zero");
         EOS_WB_ASSERT(!(lhs == std::numeric_limits<int32_t>::min() && rhs.to_i32() == -1), wasm_interpreter_exception,
      ...
      [[gnu::always_inline]] inline void operator()(const i32_div_u_t& op) {
         ...
         const auto& rhs = context.pop_operand();
         auto&       lhs = context.peek_operand().to_ui32();
         EOS_WB_ASSERT(rhs.to_ui32() != 0, wasm_interpreter_exception, "i32.div_u divide by zero");
         lhs /= rhs.to_ui32();
      }
      [[gnu::always_inline]] inline void operator()(const i32_rem_s_t& op) {
         ...
         const auto& rhs = context.pop_operand();
         auto&       lhs = context.peek_operand().to_i32();
         EOS_WB_ASSERT(rhs.to_i32() != 0, wasm_interpreter_exception, "i32.rem_s divide by zero");
         if (UNLIKELY(lhs == std::numeric_limits<int32_t>::min() && rhs.to_i32() == -1))
            lhs = 0;
         else
            lhs %= rhs.to_i32();
      }
      [[gnu::always_inline]] inline void operator()(const i32_rem_u_t& op) {
         ...
         const auto& rhs = context.pop_operand();
         auto&       lhs = context.peek_operand().to_ui32();
         EOS_WB_ASSERT(rhs.to_ui32() != 0, wasm_interpreter_exception, "i32.rem_u divide by zero");
         lhs %= rhs.to_ui32();
      }
      ...
      [[gnu::always_inline]] inline void operator()(const i64_div_s_t& op) {
         ...
         EOS_WB_ASSERT(rhs.to_i64() != 0, wasm_interpreter_exception, "i64.div_s divide by zero");
         EOS_WB_ASSERT(!(lhs == std::numeric_limits<int64_t>::min() && rhs.to_i64()
          == -1), wasm_interpreter_exception,
      ...
      [[gnu::always_inline]] inline void operator()(const i64_div_u_t& op) {
         ...
         const auto& rhs = context.pop_operand();
         auto&       lhs = context.peek_operand().to_ui64();
         EOS_WB_ASSERT(rhs.to_ui64() != 0, wasm_interpreter_exception, "i64.div_u divide by zero");
         lhs /= rhs.to_ui64();
      }
      [[gnu::always_inline]] inline void operator()(const i64_rem_s_t& op) {
         ...
         const auto& rhs = context.pop_operand();
         auto&       lhs = context.peek_operand().to_i64();
         EOS_WB_ASSERT(rhs.to_i64() != 0, wasm_interpreter_exception, "i64.rem_s divide by zero");
         if (UNLIKELY(lhs == std::numeric_limits<int64_t>::min() && rhs.to_i64() == -1))
            lhs = 0;
         else
            lhs %= rhs.to_i64();
      }
      [[gnu::always_inline]] inline void operator()(const i64_rem_u_t& op) {
         ...
         const auto& rhs = context.pop_operand();
         auto&       lhs = context.peek_operand().to_ui64();
         EOS_WB_ASSERT(rhs.to_ui64() != 0, wasm_interpreter_exception, "i64.rem_s divide by zero");
         lhs %= rhs.to_ui64();
      }
