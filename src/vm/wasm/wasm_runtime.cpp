...
namespace wasm {
   ...
   template<typename Impl>
   std::shared_ptr<wasm_instantiated_module_interface> wasm_vm_runtime<Impl>::instantiate_module(const char* code_bytes, size_t code_size ) {
      using backend_t = backend<wasm::wasm_context_interface, Impl>;
      try {
   ...
   template class wasm_vm_runtime<vm::interpreter>;
   template class wasm_vm_runtime<vm::jit>;
}//wasm