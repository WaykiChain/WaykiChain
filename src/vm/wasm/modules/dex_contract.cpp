...
void dex::dex_operator_register(wasm_context &context) {
    ...
    auto params = wasm::unpack<std::tuple<uint64_t, wasm::asset, uint64_t, uint64_t, string, string>>(context.trx.data);

    auto registrant = std::get<0>(params);
    auto fee        = std::get<1>(params);
    auto owner      = std::get<2>(params);
    auto matcher    = std::get<3>(params);
    auto portal_url = std::get<4>(params);
    ...
    WASM_ASSERT(memo.size() <= MEMO_SIZE_MAX, wasm_assert_exception, "memo.size=%d is more than %s",
                memo.size());
    ...
    DexOperatorDetail detail = { owner_name, matcher_name, portal_url, memo };
