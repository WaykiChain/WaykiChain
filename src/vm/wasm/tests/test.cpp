...
namespace wasm {
    ...
    string stToString(const string& value)
    {
        ...
        auto t = wasm::unpack<std::tuple<asset,asset,name>>(v);
    ...
    void print(const CWasmDbWrapper& cache)
    {
        ...
        for(auto iter = cache.database.begin(); iter != cache.database.end(); iter++) 
        {
            if(iter->second.size() != 16){
               std::cout << std::string("key:") << StringToHexString(iter->first,"") << std::string(" value:") 
                         << StringToHexString(iter->second,"") <<std::string("(")<<stToString(iter->second)<<std::string(")")<<std::endl ;
            }else{
               
               std::cout << std::string("key:") << StringToHexString(iter->first,"") << std::string(" value:") 
                         << StringToHexString(iter->second,"") <<std::string("(")<<accountToString(iter->second)<<std::string(")")<<std::endl ;
            }
        }    
...
int main(int argc, char** argv) {
    ...
    std::cout << std::string("1.wasm.token->create issuer:walker maximum_supply:asset{1000000000, symbol('BTC', 4)}")<<std::endl ;
    ...
    //issue
    std::cout << std::string("---------------------------------------------------------------------------")<<std::endl ;
    std::cout << std::string("2.1 wasm.token->issue to:walker quantity:asset{800000000, symbol('BTC', 4)}")<<std::endl ;
    ...
    std::cout << std::string("---------------------------------------------------------------------------")<<std::endl ;
    std::cout << std::string("2.2 wasm.token->issue to:xiaoyu quantity:asset{100000000, symbol('BTC', 4)}")<<std::endl ;
    ...
    //transfer
    std::cout << std::string("------------------------------------------------------------------------------------------")<<std::endl ;
    std::cout << std::string("4.1 wasm.token->transfer from:walker to:xiaoyu quantity:asset{100000000, symbol('BTC', 4)}")<<std::endl ;
