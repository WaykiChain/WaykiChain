...
namespace wasm { namespace rpc{
    ...
    const char *submit_wasm_contract_call_tx_rpc_help_message = R"=====(
        wasm_submitcontractcalltx "sender" "contract" "action" "data" "fee"
        ...
        "txid":       (string)
        Examples:
        > ./coind wasm_submitcontractcalltx 0-2 800-2 transfer '["0-2", "0-3", "100.00000000 WICC","transfer to bob"]'
        As json rpc call
        > curl --user myusername -d '{"jsonrpc": "1.0", "id":"curltest", "method":"wasm_submitcontractcalltx", "params":["0-2", "800-2", "transfer", '["0-2", "0-3", "100.00000000 WICC","transfer to bob"]']}' -H 'Content-Type: application/json;' http://127.0.0.1:8332
    )=====";
