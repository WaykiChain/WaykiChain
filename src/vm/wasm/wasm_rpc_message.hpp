...
namespace wasm { namespace rpc{
    ...
    const char *submit_wasm_contract_deploy_tx_rpc_help_message = R"=====(
        ...
        As json rpc call 
        > curl --user myusername -d '{"jsonrpc": "1.0", "id":"curltest", "method":"submitwasmcontractdeploytx", "params":["0-2", "0-2", "/tmp/token.wasm", "/tmp/token.bai"]}' -H 'Content-Type: application/json;' http://127.0.0.1:8332
    )=====";
    ...
    const char *get_table_wasm_rpc_help_message = R"=====(
        ...
        "more":       (bool)"
        nExamples: 
        > ./coind gettablewasm 0-2 accounts 
        As json rpc call 
        > curl --user myusername -d '{"jsonrpc": "1.0", "id":"curltest", "method":"gettablewasm", "params":["0-2", "accounts"]}' -H 'Content-Type: application/json;' http://127.0.0.1:8332
    )=====";
