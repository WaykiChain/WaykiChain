...
namespace wasm { 
   ...
   json_spirit::Value abi_serializer::_binary_to_variant( const type_name& type,  wasm::datastream<const char*>& ds, wasm::abi_traverse_context& ctx )const{
      ...
      if ( is_array(rtype) ) {
         ...
         try {
             ds >> size;
         } EOS_RETHROW_EXCEPTIONS (UNPACK_EXCEPTION, "UNPACK_EXCEPTION", "Unable to unpack size of array '%s' ", rtype.data() )
         ...
         for(decltype(size.value) i= 0; i < size; ++i ) {
             ...
             WASM_ASSERT( !v.is_null(), UNPACK_EXCEPTION, "UNPACK_EXCEPTION", "Invalid packed array '%s'", rtype.data() );
         ...
         return json_spirit::Value(std::move(vars));
          ...
          try {
            ds >>flag;
         } EOS_RETHROW_EXCEPTIONS (UNPACK_EXCEPTION, "UNPACK_EXCEPTION", "Unable to unpack presence flag of optional '%s' ", rtype.data() ) 
   ...
   json_spirit::Value abi_serializer::binary_to_variant( const type_name& type, const bytes& binary, microseconds max_serialization_time )const {
      wasm::datastream<const char*> ds( binary.data(), binary.size() );
      wasm::abi_traverse_context ctx(max_serialization_time);
      return _binary_to_variant(type, ds, ctx);
   }

   // json_spirit::Value abi_serializer::binary_to_variant( const type_name& type, wasm::datastream<const char*>& ds, microseconds max_serialization_time )const {
   //    wasm::abi_traverse_context ctx(max_serialization_time);
   //    return _binary_to_variant(type, ds, ctx);
   // }


   // json_spirit::Value&  abi_serializer::_binary_to_variant( const type_name& type, const bytes& binary, wasm::abi_traverse_context& ctx )const
   // {
   //    wasm::datastream<const char*> ds( binary.data(), binary.size() );
   //    return _binary_to_variant(type, ds, ctx);
   // }
  
   // json_spirit::Value abi_serializer::binary_to_variant( const type_name& type, const bytes& binary, wasm::abi_traverse_context& ctx )const {
   //    //wasm::abi_traverse_context ctx(max_serialization_time);
   //    wasm::datastream<const char*> ds( binary.data(), binary.size() );
   //    return _binary_to_variant(type, ds, ctx);
   // }
