...
class WasmHostMethods {
    ...
    int32_t db_store( const void* key, uint32_t key_len, const void* val, uint32_t val_len ) {  
        ...
        wasm_assert(pWasmContext->SetData(contract, k, v), StringPrint("wasm db_store SetContractData failed, key:", ToHex(k," ")).data());      //wasmContext.AppendUndo(contract, k, oldValue);
