...
public:
    CCoinTranferTx()
        : CBaseTx(COIN_TRANSFER_TX), coins(0), coinType(CoinType::WICC), feesCoinType(CoinType::WICC) {}

    CCoinTranferTx(const CBaseTx *pBaseTx): CBaseTx(COIN_TRANSFER_TX) {
        assert(COIN_TRANSFER_TX == pBaseTx->nTxType);
