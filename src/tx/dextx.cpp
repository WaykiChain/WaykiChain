...
// class CDealItemExecuter
    ...
    bool CDealItemExecuter::CalcWusdFrictionFee(uint64_t amount, uint64_t &frictionFee) {
        ...
        if (!context.pCw->sysParamCache.GetParam(DEX_FRICTION_FEE_RATIO, frictionFeeRatio))
            return context.pState->DoS(100, ERRORMSG("%s, read DEX_FRICTION_FEE_RATIO error", DEAL_ITEM_TITLE),
