        ...
        if (order_type == ORDER_MARKET_PRICE && order_side == ORDER_BUY) {
            if (asset_amount != 0)
                return context.pState->DoS(100, ERRORMSG("%s, asset amount=%llu must be 0 when order_type=%s, order_side=%s",
                        ORDER_TX_TITLE, asset_amount, kOrderTypeHelper.GetName(order_type),
            ...
            if (coin_amount != 0)
                return context.pState->DoS(100, ERRORMSG("%s, coin amount=%llu must be 0 when order_type=%s, order_side=%s",
                        ORDER_TX_TITLE, coin_amount, kOrderTypeHelper.GetName(order_type),
        ...
        if (order_type == ORDER_MARKET_PRICE) {
            if (price != 0)
                return context.pState->DoS(100, ERRORMSG("%s, price must be 0 when order_type=%s",
                        ORDER_TX_TITLE, price, kOrderTypeHelper.GetName(order_type)),
    ...
    bool CDEXSettleTx::GetDealOrder(CCacheWrapper &cw, CValidationState &state, uint32_t index, const uint256 &orderId,
        ...
        if (dealOrder.order_side != orderSide)
            return state.DoS(100, ERRORMSG("%s(), expected order_side=%s "
                    "but got order_side=%s! i=%d, orderId=%s", __func__, kOrderSideHelper.GetName(orderSide),
                    kOrderSideHelper.GetName(dealOrder.order_side), index, orderId.ToString()),
                    REJECT_INVALID,
