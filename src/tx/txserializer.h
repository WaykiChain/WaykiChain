...
void CBaseTx::SerializePtr(Stream& os, const std::shared_ptr<CBaseTx> &pBaseTx, int serType, int version) {
    ...
    switch (txType) {
        ...
        case CDP_SETTLE_INTEREST_TX:
            ::Serialize(os, (const CCDPSettleInterestTx&)tx, serType, version); break;
...
void CBaseTx::UnserializePtr(Stream& is, std::shared_ptr<CBaseTx> &pBaseTx, int serType, int version) {
    ...
    switch((TxType)nTxType) {
        ...
        case CDP_SETTLE_INTEREST_TX: {
            pBaseTx = std::make_shared<CCDPSettleInterestTx>();
            ::Unserialize(is, *((CCDPSettleInterestTx *)(pBaseTx.get())), serType, version);
            break;
        }
